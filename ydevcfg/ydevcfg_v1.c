...
#define FRAME_ADDR_OFFSET 			26
#define FRAME_READ_LEN_OFFSET 		28
...
static void wait_for_event(struct xdevcfg_drvdata *drvdata)
{
	u32 intr_status;
	unsigned long timeout;
	...
	while(!time_after(jiffies,timeout))
	{
		...
		{
			drvdata->error_status = 1;
			printk("Error occured. Flags reg: 0x%08x.\n",intr_status);
			return;
		}
	}
	printk("DMA Timeout.");
...
static void report_dma_status(struct xdevcfg_drvdata *drvdata)
{
	u32 reg;

	reg = xdevcfg_readreg(drvdata->base_address +
						XDCFG_INT_STS_OFFSET);
	printk("INTR REG: 0x%08x.\n",reg);
...
xdevcfg_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	packet_start[FRAME_ADDR_OFFSET] = 0x00000c00 + frame_base_addr;
	...
	kbuf = dma_alloc_coherent(drvdata->dev, ext_count, &dma_addr, GFP_KERNEL);
	if (!kbuf) {
		status = -ENOMEM;
		goto err_unlock;
	}

	// ALLOC buffer for the outgoing commands
	cmdbuf = dma_alloc_coherent(drvdata->dev, sizeof(packet_start), &dma_addr_write, GFP_KERNEL);
	if (!cmdbuf) {
		status = -ENOMEM;
		goto err_unlock2;
	}

	memcpy(cmdbuf,packet_start,sizeof(packet_start));

	printk("Buffers allocated. Read base: %p, Write base: %p.\n",(void*)dma_addr,(void*)dma_addr_write);
	printk("Read buffer address: %p, write buffer address: %p, user address: %p.\n",kbuf,cmdbuf,buf);
	...
	//xdevcfg_writereg(drvdata->base_address + XDCFG_INT_MASK_OFFSET,(u32) (~(XDCFG_IXR_D_P_DONE_MASK |	XDCFG_IXR_ERROR_FLAGS_MASK)));
/////////////////////////////////////////////////// CMD WRITE
	/* Initiate DMA write command */
	...
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_ADDR_OFFSET,
				//(u32)dma_addr);
				(u32)XDCFG_DMA_INVALID_ADDRESS);			
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_LEN_OFFSET, (u32)sizeof(packet_start)/4);
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_LEN_OFFSET,
				0);


	wait_for_event(drvdata);
	if (drvdata->error_status)
	{
			printk("Step 1 fault.\n");
			status = -EFAULT;
	...
	//printk("Commands sent.\n");
	//report_dma_status(drvdata);
...
///////////////////////////////////// DESYNC COMMAND
	drvdata->dma_done = 0;
	drvdata->error_status = 0;
	memcpy(cmdbuf,packet_end,sizeof(packet_end));
	/* Initiate DMA write command */
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_ADDR_OFFSET,
				(u32)dma_addr_write);
				//(u32)XDCFG_DMA_INVALID_ADDRESS);
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_ADDR_OFFSET,
				//(u32)dma_addr);
				(u32)XDCFG_DMA_INVALID_ADDRESS);			
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_LEN_OFFSET, sizeof(packet_end)/4);
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_LEN_OFFSET,
				0);
	...
	printk("Trailing ready.\n");
	report_dma_status(drvdata);

	/* If we fail to return the data to the user, then bail out. */
	printk("Writeing from address: %p.\n",kbuf+FRAME_SIZE_IN_WORDS);
	if (copy_to_user(buf, kbuf+FRAME_SIZE_IN_WORDS, count)) {
		status = -EFAULT;
		goto error;
	}
	...
	*ppos += frame_count;
	status = count;
error:
	dma_free_coherent(drvdata->dev, sizeof(packet_start), cmdbuf, dma_addr_write);
err_unlock2:
	dma_free_coherent(drvdata->dev, ext_count, kbuf, dma_addr);
err_unlock:
	mutex_unlock(&drvdata->sem);
err_clk:
	clk_disable(drvdata->clk);
