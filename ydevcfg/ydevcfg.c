...
#define XDCFG_CFG_OFFSET		0x08 /* Config register */
...
#define XDCFG_STATUS_DMA_CMD_Q_F_MASK	0x8000000
...
#define XDCFG_IXR_ERROR_FLAGS_MASK	0x0054c840
...
#define XDCFG_DMA_TX_BUF_LENGTH 4096		// 1 page
#define XDCFG_DMA_RX_BUF_LENGTH (3*1024*1024) // 3MB
...
struct xdevcfg_drvdata {
	...
	// User defined
	u32 *tx_buf;
	dma_addr_t tx_buf_phy;
	u32 *rx_buf;
	dma_addr_t rx_buf_phy;
	bool is_first_read;
};
...
// DEVC status reporter
struct devcfg_register_t{
	char *name;
	int offset;
};
static struct devcfg_register_t devcfg_regs[] = {
	{"CTRL", XDCFG_CTRL_OFFSET},
	{"LOCK",XDCFG_LOCK_OFFSET},
	{"CFG", XDCFG_CFG_OFFSET},
	{"INT_STS", XDCFG_INT_STS_OFFSET},
	{"STATUS", XDCFG_STATUS_OFFSET},
	{"DMA_SRC_ADDR", XDCFG_DMA_SRC_ADDR_OFFSET},
	{"DMA_DEST_ADDR", XDCFG_DMA_DEST_ADDR_OFFSET},
	{"DMA_SRC_LEN", XDCFG_DMA_SRC_LEN_OFFSET},
	{"DMA_DEST_LEN", XDCFG_DMA_DEST_LEN_OFFSET},
	{"MCTRL", XDCFG_MCTRL_OFFSET}
};

static void devcfg_dump(struct xdevcfg_drvdata *drvdata)
{
	int i;
	u32 reg_val;
	pr_info("DEVCFG register values:\n");

	// GET THE REGISTER VALUES AND PRINT THEM TO THE KERNEL LOG
	for(i=0;i<sizeof(devcfg_regs)/sizeof(struct devcfg_register_t); i++)
	{
		reg_val = xdevcfg_readreg(drvdata->base_address + devcfg_regs[i].offset);
		pr_info("\t%13s:\t0x%08x\n",devcfg_regs[i].name, reg_val);
	}
}
static int xPollRdy(struct xdevcfg_drvdata *drvdata, u32 timeout_ms)
{
	u32 intr_status;
	unsigned long timeout;
	...
	while(!time_after(jiffies,timeout))
	{
		...
		{
			drvdata->error_status = 1;
			pr_err("Error occured.\n");
			devcfg_dump(drvdata);
			return -1;
		}

		if ((intr_status & XDCFG_IXR_D_P_DONE_MASK) == XDCFG_IXR_D_P_DONE_MASK)
		{
			drvdata->dma_done=1;
			return 0;
		}
...
static void xInitDMA(struct xdevcfg_drvdata *drvdata, u32 wr_len32, u32 rd_len32)
{
	if(wr_len32>0)
		xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_ADDR_OFFSET,(u32)(drvdata->tx_buf_phy));
	else
		xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_ADDR_OFFSET,(u32)XDCFG_DMA_INVALID_ADDRESS);

	if(rd_len32>0)
		xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_ADDR_OFFSET, (u32)(drvdata->rx_buf_phy));			
	else
		xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_ADDR_OFFSET, (u32)XDCFG_DMA_INVALID_ADDRESS);	

	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_LEN_OFFSET, wr_len32);
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_LEN_OFFSET, rd_len32);
}
/**
* @return 0 - OK, -1 - error
*/
static int xTransfer(struct xdevcfg_drvdata *drvdata, u32 *kbuf, u32 write_len32, u32 read_len32)
{
	u32 status;
	...
	if(kbuf==NULL)
		return -1;
	if(write_len32  > XDCFG_DMA_TX_BUF_LENGTH/4 || read_len32>XDCFG_DMA_RX_BUF_LENGTH/4)
	{
		pr_err("Exceeding TX/RX buffer length.\n");
		return -1;
	}
	...
	// MASK ALL INTERRUPTS
	xdevcfg_writereg(drvdata->base_address + XDCFG_INT_MASK_OFFSET, XDCFG_IXR_ALL_MASK);
	// CLEAR ALL INTERRUPTS
	xdevcfg_writereg(drvdata->base_address + XDCFG_INT_STS_OFFSET,
					XDCFG_IXR_ALL_MASK);

	// check dma command queue
	status = xdevcfg_readreg(drvdata->base_address + XDCFG_STATUS_OFFSET);
	if((status & XDCFG_STATUS_DMA_CMD_Q_F_MASK) == XDCFG_STATUS_DMA_CMD_Q_F_MASK)
	{
		// DMA command queue is full
		pr_err("DMA cmd queue full.\n");
		return -1;
	}

	// DMA is ready
	// COPY TX DATA
	memcpy(drvdata->tx_buf,kbuf, write_len32*4);

	xInitDMA(drvdata, write_len32, 0);
	if(xPollRdy(drvdata,1000))
	{
		pr_err("Error during the write operation.\n");
		return -1;
	}
	// Read the incoming data, if requested
	if(read_len32!=0)
	{
		xInitDMA(drvdata, 0,read_len32);
		if(xPollRdy(drvdata,1000))
		{
			pr_err("Error during the read operation.\n");
			return -1;
		}
	}
	return 0;
 ...
 * returns:	Success or error status.
 */
// MODIFIED: add command write DMA operation to initiate the readback
...
0xAA995566,
0x20000000,
0x30008001, 0x0000000B, 0x20000000, // SHUTDOWN
0x30008001, 0x00000007, 0x20000000, //RCRC
...
0x30008001, 0x00000004, 0x20000000, //RCFG

0x30002001, // WR FAR
0x00000000 /////// FAR BASE ADDRESS
};
...
0x48000400, //////////////////// READ LENGTH in words including the +1 frame!!!
							//// The first frame read is dummy
0x20000000,
0x20000000,
0x20000000,
0x20000000,
...
#define FRAME_ADDR_OFFSET 			28
#define FRAME_READ_LEN_OFFSET 		4
#define FRAME_READ_LEN_MASK 		0x00FFFFFF
#define FRAME_READ_LEN_START 		0x48000000



u32 packet_end[] = { 
0x20000000,
0x30008001, 0x00000005, 0x20000000, //START
0x30008001, 0x00000007, 0x20000000, //RCRC
0x30008001, 0x0000000D, 0x20000000, //DESYNCH
0x20000000,
0x20000000,
0x20000000,
0x20000000,
...
u32 packet_read_reg[]={
 0x2800e001 ,0x20000000, 0x20000000
};

static void get_address(struct xdevcfg_drvdata *drvdata)
{/*
	struct DMABuffer buf;

	if(xInitDMABuffer(&buf, drvdata->dev, 3,1))
		return;
	memcpy(buf->tx_buf, packet_read_reg, 3*4);

	if(xTransfer(drvdata, buf,3,1))
	{
		xFreeDMABuffer(&buf);
		return;
	}

	printk("FAR value: 0x%08x.\n",*(buf->rx_buf));

	xFreeDMABuffer(&buf);
*/
}

#define FRAME_SIZE_IN_WORDS (101)
#define FRAME_SIZE_IN_BYTES (FRAME_SIZE_IN_WORDS * 4)
static ssize_t
xdevcfg_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	// word_count : words to be read
	// frame_count : frames to be read
	// ext_count : bytes to be read with the +1 starting frame to be dropped
	// ext_word_count : word to be read with the +1 starting frame to be dropped
	//u32 frame_base_addr;
	u32 frame_count;
	u32 word_count;
	u32 ext_count;
	u32 ext_word_count;
	//frame_base_addr = (u32)*ppos;
	frame_count = count/FRAME_SIZE_IN_BYTES; // ennyi frame-t akarunk olvasni 
	count = frame_count *FRAME_SIZE_IN_BYTES; // a count felesleges részét levágjuk, nem engedünk tört frame olvasást
	word_count = count / 4;
	if(drvdata->is_first_read)
	{
		ext_count = count + FRAME_SIZE_IN_BYTES;
		ext_word_count = word_count + FRAME_SIZE_IN_WORDS; 
	}
	else
	{
		ext_count = count;
		ext_word_count = word_count;
	}
	
	printk("Reading %d frames (%d bytes) with command: 0x%08x.\n",frame_count,count,FRAME_READ_LEN_START |  (FRAME_READ_LEN_MASK & word_count));
	printk("Extended byte count: %d, extended word count: %d, frame num: 0x%x.\n",ext_count, ext_word_count,*ppos);


	// modify read command with length and base address
	//packet_read[FRAME_ADDR_OFFSET] = 0x00000000 + frame_base_addr;
	packet_read[FRAME_READ_LEN_OFFSET] = FRAME_READ_LEN_START |  (FRAME_READ_LEN_MASK & ext_word_count);
	...
	if(xTransfer(drvdata, packet_read, sizeof(packet_read)/4 , ext_word_count))
	{
		printk("Failed at read.\n");
		status=-EFAULT;
		goto err_unlock;
	}
	...
	if(drvdata->is_first_read)
	{
		if (copy_to_user(buf, drvdata->rx_buf + FRAME_SIZE_IN_WORDS, count)) 
		{
			status = -EFAULT;
			goto err_unlock;
		}
	...
	{
		if (copy_to_user(buf, drvdata->rx_buf, count)) 
		{
			status = -EFAULT;
			goto err_unlock;
		}
	...
	// increasing the frame base address counter
	drvdata->is_first_read = 0;
	*ppos += frame_count;
	status = count;
...
// send packet start to get ready for the reading
	if(xTransfer(drvdata, packet_start, sizeof(packet_start)/4 , 0))
	{
		printk("Failed at opening.\n");
		status=-EFAULT;
		goto error;
	}
...
// Close read process
	// write trailin
	if(xTransfer(drvdata, packet_end, sizeof(packet_end)/4 , 0))
	{
		printk("Failed at closing.\n");
	}
...
static int xdevcfg_drv_probe(struct platform_device *pdev)
{
	...
	// USER CODE:
	// init DMA buffers
	drvdata->tx_buf = dma_alloc_coherent(&pdev->dev, XDCFG_DMA_TX_BUF_LENGTH, &(drvdata->tx_buf_phy),GFP_KERNEL);
	if(!drvdata->tx_buf)
	{
		dev_err(&pdev->dev, "Cannot allocate tx buffer for the DMA.\n");
		return -ENOMEM;
	}
	drvdata->rx_buf = dma_alloc_coherent(&pdev->dev, XDCFG_DMA_RX_BUF_LENGTH,&(drvdata->rx_buf_phy),GFP_KERNEL);
	if(!drvdata->rx_buf)
	{
		dev_err(&pdev->dev,"Cannot allocate rx buffer for the DMA.\n");
		goto usr1;
	...
	ctrlreg |= XDCFG_CTRL_PCFG_PROG_B_MASK | XDCFG_CTRL_PCAP_MODE_MASK | XDCFG_CTRL_PCAP_RATE_EN_MASK; //USER CODE: RATE EN
...
usr2:
	dma_free_coherent(&pdev->dev,XDCFG_DMA_RX_BUF_LENGTH,drvdata->rx_buf, drvdata->rx_buf_phy);
usr1:
	dma_free_coherent(&pdev->dev, XDCFG_DMA_TX_BUF_LENGTH, drvdata->tx_buf,drvdata->tx_buf_phy);	
	return retval;
}
