...
#define XDCFG_IXR_ERROR_FLAGS_MASK	0x0054c860
...
#define FRAME_READ_LEN_OFFSET 		30
...
xdevcfg_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	printk("Reading %d frames (%d bytes) frome base base address: 0x%08x, with command: 0x%08x.\n",frame_count,count,frame_base_addr,FRAME_READ_LEN_START |  (FRAME_READ_LEN_MASK & word_count));
	printk("Extended byte count: %d, extended word count: %d.\n",ext_count, ext_word_count);


	// modify read command with length and base address
	packet_start[FRAME_ADDR_OFFSET] = 0x00000000 + frame_base_addr;
	packet_start[FRAME_READ_LEN_OFFSET] = FRAME_READ_LEN_START |  (FRAME_READ_LEN_MASK & ext_word_count);
	...
	if(xTransfer(drvdata, packet_start, sizeof(packet_start)/4 , ext_word_count))
	{
		printk("Failed at step 1.\n");
		status=-EFAULT;
		goto err_unlock;
	}
